...
\section{Nuestra respuesta:}
$ejercicio$ ES $\Z$ \\
$examen$ ES $\seq{\Tipo{ejercicio}}$ \\
$resolucion$ ES $\seq{\Tipo{examen}}$ \\
$aula$ ES $\seq{\seq{\Tipo{resolucion}}}$ \\
$estudiante$ ES $\Tipo{struct}<x : \Z, y: \Z>$ \\
$nota$ ES $\Z$ 

\begin{tad}{EdR}
    \obs{estudiantes}{aula} \\
    \obs{solucion}{examen} \\
    ...
    \In aula : aula, \
    \In solucion : examen, \
    \In cantidadEstudiantes : \Z
    }{
    \Tipo{EdR}
    }
    \requiere{\norm{aula} > 0}
    \requiere{\norm{aula[0]} > 0}
    \requiere{\paraTodo{i}{\Z}{0 < i < \norm{aula} \implicaLuego \norm{aula[i]} = \norm{aula[0]}}}
    \requiere{0 \leq cantidadEstudiantes \leq \norm{aula} * (\frac{\norm{aula}}{2} + \norm{aula} \; \mod 2)}
    \requiere{\paraTodo{i}{\Z}{0 < i < \norm{solucion} \implicaLuego 0 \leq solucion[i] \leq 9}}
    \asegura{res.solucion = solucion}
    \asegura{\norm{res.estudiantes} = \norm{aula}}
    \asegura{\norm{res.estudiantes[0]} = \norm{aula[0]}}
    \asegura{\paraTodo{i}{\Z}{0 < i < \norm{res.estudiantes} \implicaLuego \norm{res.estudiantes[i]} = \norm{res.estudiantes[0]}}}
    falta hacer que res.estudiantes tenga la cantidad de estudiantes, que esten representados correctamente y que cada estudiante no tenga otro al lado
